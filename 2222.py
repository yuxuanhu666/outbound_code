...
# 计算每个箱区到泊位的最短距离（欧氏距离）
# def min_distance_to_berths(pos, berth_positions):
#     bx, by = pos
#     dists = []
#     for _, (vx, vy) in berth_positions.items():
#         d = math.hypot(bx - vx, by - vy)
#         dists.append(d)
#     return min(dists)
#
#
# block_distance_to_berth = {}
# for b, pos in block_positions.items():
#     block_distance_to_berth[b] = min_distance_to_berths(pos, berth_positions)
distance = {}
...
vessel_info = {
    ...
    'SHIP4': {'cycle_days': 7, 'berth': 'V2', 'start_time_step': 1},
    'SHIP5': {'cycle_days': 10, 'berth': 'V2', 'start_time_step': 4},
...
class YardSolver:
    ...
    def run(self):
        ...
        for window_start in range(1, T+1, window_size):
            ...
            # 性能优化
            m.Params.Threads = 4  # 使用所有核心
            m.Params.MIPFocus = 1  # 优先找可行解
            m.Params.Heuristics = 0.1  # 10% 时间用于启发式

            # 节点策略
            m.Params.NodeMethod = 1  # 对偶单纯形
            m.Params.BranchDir = 1  # 向上分支
